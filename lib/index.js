...
Interface.prototype.onconnect = function (event, from, to, msg) {
  ...
  self.ws.on('connectFailed', function(error) {
    ...
    console.info('on connectionFailed');
...
/*      if (reason.indexOf('Invalid token') != -1) {
    ...
    case 'serverHello':
      if (self.is('_connected')) {
        ...
        console.error('State is not connected!');
      }
      ...
      switch (policy) {
        ...
        case 'any':
          ...
          for(var i = 0; i < resource.length; i++) {
            ...
            (function(item) {
              self.emit('try:' + item.tType, item, function (err, credentials) {
                console.info('Try callback result: ', err, item, credentials);
                ...
                item.probeData.samples = credentials;
