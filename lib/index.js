  ...
  switch (message.msg.oid) {
    ...
    case 'rpcResp':
      ...
      if (self.is('_ready')) {
        ...
        var criteria = message.msg.session_id + ':' + message.msg.namespace + ':' + message.msg.call;

        if (typeof self.rpcCallbacks[criteria] == 'function') {
          self.rpcCallbacks[criteria](message);
          //delete self.rpcCallbacks[message.msg.session_id];
        } else {
          debug.error('RPC callback for session: '+ criteria +' not found!');
        }
      }
...
Interface.prototype.rpc = function() {
  ...
  var webResourceId = params.webResourceId;
  ...
  /* save callback */
  self.rpcCallbacks[sessionId + ':' + command] = callback;

  switch(command) {
    case 'auth_client_plugin:process_auth':
      self.ws.sendAuthRequest(sessionId, clientId, providerId, userId, resources);
      break;
    case 'auth_client_plugin:cancel_auth':
      self.ws.sendCancelAuthRequest(sessionId, clientId, providerId, userId, resources);
      break;
    case 'app2user_plugin:get_user':
      self.ws.sendGetUserByAppIdRequest(sessionId, userId, resources);
      break;
    case 'crt2user_plugin:get_user':
      self.ws.sendGetUserByCertIdRequest(sessionId, userId, resources);
      break;
    case 'auth_client_plugin:get_web_resource_secret':
      self.ws.sendGetWebResourceSecretRequest(sessionId, webResourceId, providerId, userId, resources);
      break;
    case 'identify_client_plugin:process_auth':
      self.ws.sendIdentifyByPhotoRequest(sessionId, photos, resources);
      break;
    default:
...
Interface.prototype.finish = function(sessionId) {
  var self = this;
  delete self.rpcCallbacks[sessionId + ':auth_client_plugin:process_auth'];
  delete self.rpcCallbacks[sessionId + ':auth_client_plugin:cancel_auth'];
  delete self.rpcCallbacks[sessionId + ':app2user_plugin:get_user'];
  delete self.rpcCallbacks[sessionId + ':crt2user_plugin:get_user'];
  delete self.rpcCallbacks[sessionId + ':auth_client_plugin:get_web_resource_secret'];
  delete self.rpcCallbacks[sessionId + ':identify_client_plugin:process_auth'];
};
