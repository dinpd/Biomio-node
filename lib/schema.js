...
Schema.prototype.getAuthRequest = function (token, clientId, sessionId, keyValueDict) {
...
/*
  tryReq:
  {
    tType: 'text_credentials',
    resource: { rProperties: '', rType: 'user_input'},
    samples: 1,
    interactive: true,
    tryId: 'bcea2314##DA2lGHh84OCTEm6FAAAB',
    sessionId: 'DA2lGHh84OCTEm6FAAAB',
    policy: 'all',
    oid: 'textCredentialsSamples'
  }
*/
Schema.prototype.getTryRequest = function(token, result) {
  ...
  request.header.token = token;
  request.msg.probeData.oid = result.tryReq.oid;
  request.msg.probeData.samples = result.tryResult;
  request.msg.tType = result.tryReq.tType;
  request.msg.try_id = result.tryReq.tryId;
