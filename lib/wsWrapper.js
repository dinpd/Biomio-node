...
Ws.prototype.sendAuthRequest = function(sessionId, clientId, userId, resources) {
  var self = this;
  var request = self.schema.getAuthRequest(self.connectionData.token, clientId, sessionId, resources, {email: userId, auth_code: 'NO_REST'})
  self.send(request);
};

Ws.prototype.sendCancelAuthRequest = function(sessionId, clientId, userId, resources) {
  var self = this;
  var request = self.schema.getCancelAuthRequest(self.connectionData.token, clientId, sessionId, resources, {email: userId, auth_code: 'NO_REST'})
  self.send(request);
};

Ws.prototype.sendGetUserRequest = function(sessionId, clientId, userId, resources) {
  var self = this;
  var request = self.schema.getGetUserRequest(self.connectionData.token, clientId, sessionId, resources, {})
  self.send(request);
};
