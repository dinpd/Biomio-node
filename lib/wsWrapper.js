...
Ws.prototype.sendAuthRequest = function(sessionId, clientId, providerId, userId, resources) {
  var self = this;
  var request = self.schema.getAuthRequest(self.connectionData.token, providerId, userId, sessionId, resources, {email: clientId, auth_code: 'NO_REST'});
  self.send(request);
};

Ws.prototype.sendCancelAuthRequest = function(sessionId, clientId, providerId, userId, resources) {
  var self = this;
  var request = self.schema.getCancelAuthRequest(self.connectionData.token, providerId, userId, sessionId, resources, {email: clientId, auth_code: 'NO_REST'});
  self.send(request);
};

Ws.prototype.sendGetWebResourceSecretRequest = function(sessionId, webResourceId, providerId, userId, resources) {
  var self = this;
  var request = self.schema.getGetWebResourceSecretRequest(self.connectionData.token, userId, sessionId, resources, {web_resource_id: webResourceId, provider_id: providerId});
  self.send(request);
};

Ws.prototype.sendIdentifyByPhotoRequest = function(sessionId, photos, resources) {
  var self = this;
  var data = {
    count: photos.length.toString()
  };
  for(var i = 0; i < photos.length; i++) data['image' + (i+1)] = photos[i];


  var request = self.schema.getIdentifyByPhotoRequest(self.connectionData.token, sessionId, resources, data);
  self.send(request);
};

Ws.prototype.sendGetUserByAppIdRequest = function(sessionId, userId, resources) {
  var self = this;
  var request = self.schema.getGetUserByAppIdRequest(self.connectionData.token, userId, sessionId, resources, {})
  self.send(request);
};

Ws.prototype.sendGetUserByCertIdRequest = function(sessionId, userId, resources) {
  var self = this;
  var request = self.schema.getGetUserByCertIdRequest(self.connectionData.token, userId, sessionId, resources, {})
  self.send(request);
};
