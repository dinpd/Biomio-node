...
var STATE_INITIALIZE = 'initialize_state',
    ...
    STATE_RPC_CALL_GET_USER_BY_APP = 'rpc_call_get_user_by_app'
...
function ClientInterface(client_key, options, ready_callback) {
    ...
    this._state_machine = StateMachine.create({
        ...
        events: [
            ...
            {name: '_get_user_by_app', from: STATE_READY, to: STATE_RPC_CALL_GET_USER_BY_APP},
        ...
        callbacks: {
            ...
            on_get_user_by_app: this._onGetUserByApp,
...
ClientInterface.prototype._onGetUserByApp = function (event, from, to, msg, self) {
    logger.log('info', msg);
    internal_state_machine.check_if_user_exists(self._client_key, self._get_rpc_response_callback());
};
...
ClientInterface.prototype._get_rpc_response_callback = function () {
    ...
    return function (response) {
        ...
        else if (self._state_machine.is(STATE_RPC_CALL_GET_USER_BY_APP)) {
            console.log('aaaa')
        }
...
ClientInterface.prototype.get_user_by_app = function (response_callback) {
    if (typeof response_callback != 'undefined' && response_callback) {
        this._client_callback = response_callback;
    } else {
        this._client_callback = null;
    ...
    if (this._validate_state()) {
        this._state_machine._get_user_by_app('Getting user by appId - ' + this._client_key + ' exists.', this);
    }
};
