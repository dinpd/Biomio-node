...
var STATE_INITIALIZE = 'initialize_state',
    ...
    STATE_RPC_CALL_GET_USER = 'rpc_call_get_user',
    STATE_FINISH = 'finish_state';

function ClientInterface(client_key, app_id, options, ready_callback) {
    ...
    this._state_machine = StateMachine.create({
        ...
        events: [
            ...
            {
                name: '_ready',
                from: [STATE_INITIALIZE, STATE_RPC_CALL_AUTH, STATE_RPC_CALL_USER_CHECK, STATE_RPC_CALL_GET_USER],
                to: STATE_READY
            },
            {name: '_rpc_auth', from: [STATE_READY, STATE_RPC_CALL_GET_USER], to: STATE_RPC_CALL_AUTH},
            {name: '_check_user', from: STATE_READY, to: STATE_RPC_CALL_USER_CHECK},
            {name: '_get_user', from: STATE_READY, to: STATE_RPC_CALL_GET_USER},
...
ClientInterface.prototype._onGetUser = function (event, from, to, msg, self) {
    logger.log('info', msg);
    internal_state_machine.get_user(self._app_id, self._get_rpc_response_callback());
};
...
ClientInterface.prototype._get_rpc_response_callback = function () {
    ...
    return function (response) {
        ...
        if ('error' in response) {
        ...
        else if (self._state_machine.is(STATE_RPC_CALL_GET_USER)) {
            var user = response.keys[0] == 'user' ? response.values[0] : undefined;
            if (!user) {
                result = '';
            } else {
                self._on_behalf_of = user;
                self._state_machine._rpc_auth("Start authentication now", self);
                internal_state_machine.subscribe_for_responses(self._on_behalf_of, self._get_rpc_response_callback());
                result = user;
            ...
            if (response.keys.indexOf('error') != -1) {
              ...
              self._msg = response.values[0];
              self._result = false;
            } else if (response.keys.indexOf('resut') != -1) {
                self._result = true;
...
ClientInterface.prototype.get_user = function (response_callback) {
    if (typeof response_callback != 'undefined' && response_callback) {
        this._client_callback = response_callback;
    } else {
        this._client_callback = null;
    ...
    if (this._validate_state()) {
        this._state_machine._get_user('Getting user by appId - ' + this._app_id, this);
    }
};
